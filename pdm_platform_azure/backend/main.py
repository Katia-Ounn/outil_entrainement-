...
from rnn_demo import run_demo as run_rnn_demo
from lstm_demo import run_demo as run_lstm_demo
from transformer_demo import run_demo as run_transformer_demo
...
# ═══════════════════════════════════════════════════════════════
# 🎓 DÉMO RNN — Avec vraies données Machine 99
# ═══════════════════════════════════════════════════════════════
class RNNDemoRequest(BaseModel):
    layers:        List[int] = Field([4],  description="Liste des unités par couche (1 à 3 couches)")
    batch_size:    int   = Field(4,  ge=2, le=8,  description="Taille du mini-batch pour visualisation")
    seq_length:    int   = Field(3,  ge=2, le=6,  description="Longueur de séquence en heures")
...
def rnn_demo_run(req: RNNDemoRequest):
    """
    Démonstration pédagogique : utilise le vrai pipeline Machine 99,
    extrait un petit batch, et montre toutes les étapes d'un RNN multi-couches.
    """
    try:
        pipeline = get_pipeline_sync(machine_id=req.machine_id, lookback=24)
        if pipeline is None or pipeline.X_train is None:
            raise ValueError(
                "Pipeline non initialisé. Lance d'abord l'ingestion dans l'onglet Ingestion."
            )
        ...
        import traceback
        traceback.print_exc()
        raise HTTPException(status_code=500, detail=f"Erreur démo : {e}")


# ═══════════════════════════════════════════════════════════════
# 🧬 DÉMO LSTM — Avec vraies données Machine 99 et les 4 portes
# ═══════════════════════════════════════════════════════════════
class LSTMDemoRequest(BaseModel):
    layers:        List[int] = Field([4],  description="Liste des unités par couche LSTM (1 à 3 couches)")
...
def lstm_demo_run(req: LSTMDemoRequest):
    """
    Démonstration pédagogique LSTM : forget, input, candidate, output gates + cell state.
    """
    try:
        pipeline = get_pipeline_sync(machine_id=req.machine_id, lookback=24)
        if pipeline is None or pipeline.X_train is None:
...
# ═══════════════════════════════════════════════════════════════
# 🤖 DÉMO TRANSFORMER — Encoder + Decoder + Multi-Head Attention
# ═══════════════════════════════════════════════════════════════
class TransformerDemoRequest(BaseModel):
    d_model:       int   = Field(8,  ge=4, le=32, description="Dimension des embeddings (multiple de n_heads)")
    n_heads:       int   = Field(2,  ge=1, le=4,  description="Nombre de têtes d'attention")
    d_ff:          int   = Field(16, ge=8, le=64, description="Dimension feed-forward")
    batch_size:    int   = Field(4,  ge=2, le=8)
    seq_length:    int   = Field(3,  ge=2, le=6)
...
def transformer_demo_run(req: TransformerDemoRequest):
    """
    Démonstration pédagogique Transformer : encoder + decoder + multi-head attention.
    Calcule positional encoding, Q/K/V, attention scores, softmax, FFN, layer norm.
    """
    try:
        pipeline = get_pipeline_sync(machine_id=req.machine_id, lookback=24)
        if pipeline is None or pipeline.X_train is None:
