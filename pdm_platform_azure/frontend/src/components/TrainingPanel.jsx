...
function ArchitectureVisualizer({
  ...
  return (
    <div className="rounded-xl border-2 overflow-hidden"
      style={{ borderColor: archColor, background: 'linear-gradient(135deg, #0f1117, #1a1d2e)' }}>
      ...
      <svg viewBox={`0 0 ${W} ${H}`} className="w-full" style={{ maxHeight: 420 }}>
        <defs>
          ...
          <marker id="archarrow" markerWidth="5" markerHeight="5" refX="4" refY="2.5"
            ...
            <path d="M0,0 L0,5 L5,2.5 z" fill="#8a8d9f"/>
          </marker>
        </defs>
        ...
        <g>
          ...
          <rect x={inputX - 50} y={50}
            ...
            fill="#0d2a4a" stroke="#4fc3f7" strokeWidth={1.5}/>
        ...
        {inputYs.map((y, i) => (
          <g key={`in-${i}`}>
            <rect x={inputX - 14} y={y - 9} width={28} height={18} rx={3}
              fill="#1a3a5c" stroke="#4fc3f7" strokeWidth={1}/>
            ...
            return (
              ...
              >
                <circle cx={layerXs[L]} cy={y} r={11}
                  fill={isDropped ? '#1a1d2e' : archColor + '30'}
                  stroke={isDropped ? '#4a4d6a' : archColor}
        ...
        <g transform={`translate(${W/2 - 250}, ${H - 18})`}>
          <rect x={0} y={-8} width={500} height={16} rx={8}
            fill="#1a1d2e" stroke="#2a2d45"/>
          <circle cx={20} cy={0} r={4} fill={archColor + '30'} stroke={archColor}/>
          <text x={30} y={3} fontSize={9} fill="#8a8d9f" fontFamily="monospace">neurone actif</text>
          <circle cx={140} cy={0} r={4} fill="#1a1d2e" stroke="#4a4d6a" strokeDasharray="2,2"/>
          <text x={150} y={3} fontSize={9} fill="#8a8d9f" fontFamily="monospace">neurone "dropé"</text>
          <text x={260} y={3} fontSize={9} fill="#4fc3f7" fontFamily="monospace">📦 batch={batchSize}</text>
          <text x={360} y={3} fontSize={9} fill="#81c784" fontFamily="monospace">⏱ window={lookback}h</text>
        </g>
      </svg>

      {/* Note pédagogique sur la couche Dense */}
      <div className="px-4 py-2.5 border-t" style={{ borderColor: '#2a2d45', background: '#0a0c14' }}>
    ...
    <div className="grid grid-cols-5 gap-6">
      ...
      <div className="col-span-2 space-y-4">
        ...
        <div className="rounded-xl border p-4 space-y-3" style={{ background:'#1a1d2e', borderColor:'#2a2d45' }}>
          ...
          <div>
            <label className={lblCls} style={{ color:'#8a8d9f' }}>
            ...
            <div className="grid grid-cols-5 gap-1.5 mb-2">
              {[12, 24, 48, 72, 168].map(h => (
                <button key={h} onClick={() => setLookback(h)}
                  ...
                  style={{
                    background: lookback === h ? '#0d2a1a' : '#232640',
                    borderColor: lookback === h ? '#81c784' : '#2a2d45',
                    color: lookback === h ? '#81c784' : '#8a8d9f',
                  }}>
                  {h}h
                </button>
              ))}
            ...
            <p className="text-xs mt-1" style={{ color: '#4a4d6a' }}>
        ...
        {mode === 'auto' && trainingStatus === 'idle' && (
          <div className="rounded-xl border p-4"
            style={{ background:'#0f1117', borderColor:'#81c78440' }}>
            <div className="flex items-start gap-2">
              <BrainCircuit size={18} style={{ color:'#81c784', flexShrink:0, marginTop:2 }}/>
              <div>
                ...
                <p className="text-xs mt-1" style={{ color:'#8a8d9f' }}>
                  ...
                  = <b style={{color:'#81c784'}}>{maxTrials * cvFolds * trialEp} époques</b> de recherche.
                </p>
              </div>
            </div>
          </div>
